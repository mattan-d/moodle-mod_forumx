 ...
 *
 * @module    mod_forumx/forumx
 * @package   mod_forumx
 * @copyright 2020 onwards MOFET
 * @license   http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 * @since     3.1
 */
define(['jquery', 'jqueryui', 'core/ajax', 'core/str', 'mod_forumx/accessibility', 'mod_forumx/stagebutton', 'mod_forumx/postgallery', 'mod_forumx/ga', 'mod_forumx/ouilsimplemenu', 'mod_forumx/toast'], function ($, jqui, ajax, s, acc, sb, pgl, fga) {
    ...
    var toggleDiscussion = function () {
        ...
        if ($(this).hasClass('d_opened')) {
            discussion.find('.forumx_post').addClass('closed_post');
            newText = PARAMS.str.openDiscussionThread;
            setPostAria(discussion, false);
        } else {
            discussion.find('.forumx_post').removeClass('closed_post');
    ...
    var togglePost = function (e) {
        ...
        setDiscussionButton(pContainer.closest('.forumx_discussion'));
    };
    ...
    var markPostAsRead = function (postId) {
        var promise = ajax.call([{
            methodname: 'mod_forumx_single_read',
            args: {
                forum: PARAMS.forum,
                post: postId
            }
    ...
    var markAllPostsAsRead = function (discussion) {
        ...
        var promise = ajax.call([{
            methodname: 'mod_forumx_multiple_read',
            args: {
                forum: PARAMS.forum,
    ...
    var setDiscussionButton = function (discussion) {
        ...
        discussion.find('.forumx_post').each(function () {
            posts++;
    ...
    var flagPost = function (e) {
        ...
        var promise = ajax.call([{
            methodname: 'mod_forumx_flag_post',
            args: {
                action: action,
                post: postId,
...
//        }
        ...
        //    acc.setWait(trigger, true);
        var promise = ajax.call([{
            methodname: 'mod_forumx_like_post',
            args: {
                postid: postId,
        ...
        promise[0].done(function (callResult) {
            ...
            if (callResult[0].action == 'like') {
                node.addClass('liked');
            } else {
                node.removeClass('liked');
            }

            node.find('.bubble').text(callResult[0].likes);
    ...
    var recommendPost = function (e) {
        ...
        acc.setWait(node, true);
        var promise = ajax.call([{
            methodname: 'mod_forumx_recommend_post',
            args: {
                action: action,
                post: postId,
                discussion: discussionId
    ...
    var getDiscussion = function (node, idOnly) {
        var discussion = node.closest('.forumx_discussion');
        if (idOnly) {
            return discussion.attr('data-discussionid');
        } else {
            return discussion;
        }
    };
    ...
    var sendQuickReply = function () {
        ...
        bindResult('reply');
        var promise = ajax.call([{
            methodname: 'mod_forumx_add_quick_discussion_post',
            args: {
            ...
            if (!callResult.error) {
                ...
                $('#quickpostmask').removeClass('active').delay(1).queue(function () {
                    ...
                    if (callResult.errortype == 'locked') {
                        var discussion = getDiscussion(postParent),
                            lockButton = discussion.find('.of_post_icons button[data-actiontype="lock"]');
    ...
    var getReplyParent = function (node, onlyId) {
        ...
        if (node.hasClass('firstpost') || node.hasClass('postlevel1')) {
            ...
            parentNode = node.closest('.indentlevel1').find('.forumx_post.postlevel1');
        }
        if (onlyId) {
    ...
    var bindResult = function (actionType) {
        if (actionType == 'discussion') {
            ...
            PARAMS.ajaxCall.methodName = 'mod_forumx_add_quick_discussion';
        } else if (actionType == 'reply') {
            PARAMS.ajaxCall.targetId = 'quickpostmask';
            PARAMS.ajaxCall.methodName = 'mod_forumx_add_quick_discussion_post';
        }
    ...
    var appendPost = function (newPost, parentPost) {
        if (parentPost.hasClass('firstpost')) {
            ...
            newPost.find('.forumx_post').addClass('lastpost');
    ...
    var setRepliesCount = function (discussion) {
        var replies = discussion.find('.forumx_post').length - 1, replyStr = '', replyNum = '';
        if (replies > 0) {
    ...
    var countChildren = function (postNode) {
        ...
        if (postNode.hasClass('firstpost')) {
            posts = getDiscussion(postNode).find('.forumx_post').length - 1;
        } else {
            posts = postNode.closest('.indent').find('.forumx_post').length - 1;
        }
        return posts;
    };
    ...
    var deletePosts = function () {
        var promise = ajax.call([{
            methodname: 'mod_forumx_delete_post',
            args: {
        ...
        promise[0].done(function (callResult) {
            ...
            PARAMS.delNode.node.animate({height: 0}, 500).delay(50).queue(function () {
                ...
                if (!PARAMS.delNode.isDiscussion) {
                    ...
                    if (hasLastPost) {
                        discussion.find('.forumx_post').last().addClass('lastpost');
                    }
                } else {
                    PARAMS.discussions--;
    ...
    var runSubscribeDiscussion = function (node) {
        ...
        var promise = ajax.call([{
            methodname: 'mod_forumx_subscribe_discussion',
            args: {
                'discussion': discussion,
                'subscribe': action
            }
        ...
        promise[0].done(function (callResult) {
            ...
            // No errors, but couldn't subscribe.
                ...
                // The user is already subscribed to the whole forum.
                    ...
                    $('.sub_discussion > div').remove();
                    subNode.attr('data-actiontype', 'unsubscribe').attr('data-buttonstate', 'on').attr('aria-label', PARAMS.str.subscribeForumNoLabel);
        ...
        promise[0].done(function (callResult) {
            ...
            if (callResult == true) {
                ...
                if (action == 'subscribe') {
                    ...
                    newAction = 'subscribe';
                    newStatus = 'off';
                    $('.forumx_post .sub_discussion').each(function () {
    ...
    var resetDigestTree = function () {
        if (PARAMS.settingType == 'block') {
            ...
            if (fromClass !== null) {
                ...
                activeMode.html('<a href="' + M.cfg.wwwroot + '/mod/forumx/maildigest.php?id=' + PARAMS.forum +
            ...
            if (fromClass !== null) {
                ...
                parentNode.html('<a href="' + M.cfg.wwwroot + '/mod/forumx/maildigest.php?id=' + PARAMS.forum +
    ...
    var runTrackForum = function (node) {
        ...
        if (action == 'track') {
            $('#region-main .discussionslist .forumx_discussion').each(function () {
        ...
        if (add) {
            if (!$.isEmptyObject(newPosts)) {
                ...
                $.each(newPosts, function (discussionId, posts) {
                    var discussion = $('#discussion' + discussionId + ' .forumx_post.firstpost .of_post_title')
    ...
    var lockDiscussion = function (discussion, lock, newButton, lockButton, lockIcon) {
        ...
        if (typeof lockButton == 'undefined' || lockButton.length == 0) {
            ...
            if (lock) {
                lockButton.removeClass('forumx_lock').addClass('forumx_unlock').attr('data-action', 'unlock').attr('title', PARAMS.str.unlockDiscussion);
            } else {
                lockButton.removeClass('forumx_unlock').addClass('forumx_lock').attr('data-action', 'lock').attr('title', PARAMS.str.lockDiscussion);
    ...
    var callLockDiscussion = function (e) {
        ...
        acc.setWait(node, true);
        var promise = ajax.call([{
            methodname: 'mod_forumx_lock_discussion',
            args: {
    ...
    var sendForward = function () {
        ...
        var promise = ajax.call([{
            methodname: 'mod_forumx_quick_forward_post',
            args: {
    ...
    var sendDiscussion = function () {
        ...
        var promise = ajax.call([{
            methodname: 'mod_forumx_add_quick_discussion',
            args: args
        }]);
        promise[0].done(function (callResult) {
            ...
            var discussionId;
            $(newDiscussion).find('div.forumx_discussion').each(function () {
    ...
    var initReplyDialog = function () {
        ...
        $('#region-main').on('click', 'button.of_reply', function (e) {
            ...
            if ($(this).hasClass('wait_for_dialog')) {
                ...
                $(this).addClass('wait_for_dialog');
                openQuickReply($(this).closest('.forumx_post'));
            }
        });
    ...
    var closeQuickReply = function (fast) {
        ...
        PARAMS.currentDialog = null;
        var button = node.prev('.forumx_post').find('.post_reply_button');
    ...
    var findPostImages = function (node) {
        var path = '.forumx_post';
        if (node) {
            path = node + ' .forumx_post';
        }
        $(path).each(function () {
    ...
    var initFindUser = function () {

        var el = $('.find_user_list'),
            courseid = el.data('courseid');


        $(document).on('click', '.find_user_list li', function () {
            var userid = $(this).data('userid'),
                username = $(this).data('username');

            //var caretPos_old = textelement[0].selectionStart;
            var textAreaTxt = textelement.val();
            var txtToAdd = '{' + username + ', ' + userid + '}';
            var caretPos = textAreaTxt.lastIndexOf('@');

            textelement.val(textAreaTxt.substring(0, caretPos) + txtToAdd);
            textelement.focus();
        });
        ...
        $(document).on('keypress', 'textarea', function (e) {
            ...
            var promise = ajax.call([{
                methodname: 'mod_forumx_find_user',
                args: {
                    typing: $(this).val(),
                    courseid: courseid
                }
            ...
            promise[0].done(function (response) {
                if (!response.error) {
                    $('.find_user_list li').remove();
                    response.forEach(function (user) {
                        el.append('<li data-username="' + user.username + '" data-userid="' + user.id + '">' + user.username + '</li>');
                    });
                }
            }).fail(function () {
                //console.log('FAILED');
    ...
    return {
        init: function (params) {
            ...
            PARAMS.singlePage = PARAMS.forumPage == 0 && $('.forumx_paging').length == 0;
            ...
            $('#region-main').on('click', 'button.forumx_recommend', recommendPost);
            $('#region-main').on('click', 'button.forumx_like', likePost);
            ...
            $('#region-main').on('click', '.simpleactionmenu a[data-action="delete"]', function (e) {
                e.preventDefault();
                callDeletePost($(this).closest('.forumx_post'));
            });
            ...
            $('#region-main').on('click', '.simpleactionmenu a[data-action="forward"]', function (e) {
                e.preventDefault();
                callForward($(this).attr('href'), $(this).closest('.forumx_post'));
            });
